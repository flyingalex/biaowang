<div class="sidebar">
    <div class="user clearfix">
        <div class="user-portrait">
            <img class="portrait-img" src="/images/test/u70.png">
        </div>
        <div class="user-info">
            <div class="user-name">李四</div>
            <div class="user-role">超级管理员</div>
        </div>
    </div>
    <ul class="menu">
        <li class="sub-menu active-menu">
            <div href="#" class="dropdown-btn">
                <img src="/images/icon/" class="dropdown-btn-icon">
                <a href="#" class="dropdown-btn-title">微官网</a>
            </div>
            <ul class="sub-menu-item">
                <li class="sub-menu-link-wrap">
                    <a href="#" class="sub-menu-link active-link">广告图片</a>
                </li>
                <li class="sub-menu-link-wrap">
                    <a href="#" class="sub-menu-link">标题设置</a>
                </li>
                <li class="sub-menu-link-wrap">
                    <a href="#" class="sub-menu-link">干货管理</a>
                </li>
                <li class="sub-menu-link-wrap">
                    <a href="#" class="sub-menu-link">添加干货</a>
                </li>
            </ul>
        </li>
        <li class="sub-menu">
            <div href="#" class="dropdown-btn">
                <img src="/images/icon/" class="dropdown-btn-icon">
                <a href="#" class="dropdown-btn-title">微投票</a>
            </div>
            <ul class="sub-menu-item">
                <li class="sub-menu-link-wrap">
                    <a href="#" class="sub-menu-link">创建项目</a>
                </li>
                <li class="sub-menu-link-wrap">
                    <a href="#" class="sub-menu-link">管理项目</a>
                </li>
                <li class="sub-menu-link-wrap">
                    <a href="#" class="sub-menu-link">添加内容</a>
                </li>
                <li class="sub-menu-link-wrap">
                    <a href="#" class="sub-menu-link">管理内容</a>
                </li>
            </ul>
        </li>
        <li class="sub-menu">
            <div href="#" class="dropdown-btn">
                <img src="/images/icon/" class="dropdown-btn-icon">
                <a href="#" class="dropdown-btn-title">微相册</a>
            </div>
            <ul class="sub-menu-item">
                <li class="sub-men-link-wrap">
                    <a href="#" class="sub-menu-link">相片上传</a>
                </li>
                <li class="sub-men-link-wrap">
                    <a href="#" class="sub-menu-link">视频上传</a>
                </li>
                <li class="sub-men-link-wrap">
                    <a href="#" class="sub-menu-link">创建相册</a>
                </li>
                <li class="sub-men-link-wrap">
                    <a href="#" class="sub-menu-link">管理相册视频
                    </a>
                </li>
                <li class="sub-men-link-wrap">
                    <a href="#" class="sub-menu-link">广告图片</a>
                </li>
            </ul>
        </li>
        <li class="sub-menu">
            <div href="#" class="dropdown-btn">
                <img src="/images/icon/" class="dropdown-btn-icon">
                <a href="#" class="dropdown-btn-title">系统管理</a>
            </div>
            <ul class="sub-menu-item">
                <li class="sub-men-link-wrap">
                    <a href="#" class="sub-menu-link">用户管理</a>
                </li>
                <li class="sub-men-link-wrap">
                    <a href="#" class="sub-menu-link">新闻管理</a>
                </li>
            </ul>
        </li>
    </ul>
</div>